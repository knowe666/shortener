...
func NewConfig() (*Config, error) {
	...
	authSecret := flag.String("s", "", "секретный ключ для подписи cookie")
	flag.Parse()
	...
	if authSecretEnv := os.Getenv("AUTH_SECRET"); authSecretEnv != "" {
		*authSecret = authSecretEnv
	}
...
func (c *Config) Validate() error {
	...
	if c.AuthSecret == "" {
		return fmt.Errorf("auth secret cannot be empty, use -s or AUTH_SECRET")
	}
